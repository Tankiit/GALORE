...
# Keep the original MASCSOptimized class as is
class MASCSOptimized:
    def __init__(self, dataset, budget_percentage, feature_extractor, num_classes,
        ...
        # Calculate budget as percentage of dataset size
        self.budget = int(len(dataset) * budget_percentage / 100)
...
# Enhanced TIMM model wrapper with more flexibility
class TIMMModel(nn.Module):
    """Enhanced wrapper for TIMM models with feature extraction capability"""
    def __init__(self, model_name='resnet18', num_classes=10, pretrained=True, drop_rate=0.0):
        ...
        # Get feature dimension and classifier
        self.feature_dim = self.model.num_features if hasattr(self.model, 'num_features') else self.model.get_classifier().in_features

        # Store the classifier separately
...
# Enhanced Dataset Manager with more datasets
class DatasetManager:
    """Enhanced manager for handling multiple datasets"""
    def __init__(self):
        self.available_datasets = {
            ...
            # Additional datasets
            'svhn': self.get_svhn_datasets,
            'food101': self.get_food101_datasets,
            'flowers102': self.get_flowers102_datasets,
            'caltech101': self.get_caltech101_datasets,
            'caltech256': self.get_caltech256_datasets,
            'stl10': self.get_stl10_datasets,
            'eurosat': self.get_eurosat_datasets,
            'dtd': self.get_dtd_datasets,
            'gtsrb': self.get_gtsrb_datasets,
            'kmnist': self.get_kmnist_datasets,
            'emnist': self.get_emnist_datasets,
        }
    ...
    def get_svhn_datasets(self, data_dir='./data'):
        """Get SVHN datasets"""
        train_transform = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.ToTensor(),
    ...
    # Add placeholder methods for other datasets (can be implemented later)
    def get_food101_datasets(self, data_dir='./data', size=224):
        """Get Food-101 datasets"""
        raise NotImplementedError("Food-101 dataset not yet implemented")

    def get_flowers102_datasets(self, data_dir='./data', size=224):
        """Get Oxford Flowers 102 datasets"""
        raise NotImplementedError("Flowers102 dataset not yet implemented")

    def get_caltech101_datasets(self, data_dir='./data', size=224):
        """Get Caltech-101 datasets"""
        raise NotImplementedError("Caltech101 dataset not yet implemented")

    def get_caltech256_datasets(self, data_dir='./data', size=224):
        """Get Caltech-256 datasets"""
        raise NotImplementedError("Caltech256 dataset not yet implemented")

    def get_stl10_datasets(self, data_dir='./data'):
        """Get STL-10 datasets"""
        raise NotImplementedError("STL-10 dataset not yet implemented")

    def get_eurosat_datasets(self, data_dir='./data', size=64):
        """Get EuroSAT datasets"""
        raise NotImplementedError("EuroSAT dataset not yet implemented")

    def get_dtd_datasets(self, data_dir='./data', size=224):
        """Get Describable Textures Dataset (DTD)"""
        raise NotImplementedError("DTD dataset not yet implemented")

    def get_gtsrb_datasets(self, data_dir='./data', size=32):
        """Get German Traffic Sign Recognition Benchmark"""
        raise NotImplementedError("GTSRB dataset not yet implemented")

    def get_kmnist_datasets(self, data_dir='./data'):
        """Get Kuzushiji-MNIST datasets"""
        raise NotImplementedError("KMNIST dataset not yet implemented")

    def get_emnist_datasets(self, data_dir='./data', split='balanced'):
        """Get Extended MNIST datasets"""
        raise NotImplementedError("EMNIST dataset not yet implemented")

    def get_imagenet_datasets(self, data_dir='./data', size=224):
        """Get ImageNet datasets"""
        raise NotImplementedError("ImageNet requires manual setup and large storage")

# Enhanced Model Manager with more models
class ModelManager:
    """Enhanced manager for handling multiple models from TIMM"""
    def __init__(self):
        self.available_models = {
            # ResNet family
            'resnet18': lambda num_classes: TIMMModel('resnet18', num_classes),
            'resnet34': lambda num_classes: TIMMModel('resnet34', num_classes),
            'resnet50': lambda num_classes: TIMMModel('resnet50', num_classes),
            'resnet101': lambda num_classes: TIMMModel('resnet101', num_classes),
            'resnet152': lambda num_classes: TIMMModel('resnet152', num_classes),
            'wide_resnet50_2': lambda num_classes: TIMMModel('wide_resnet50_2', num_classes),
            'wide_resnet101_2': lambda num_classes: TIMMModel('wide_resnet101_2', num_classes),
            'resnext50_32x4d': lambda num_classes: TIMMModel('resnext50_32x4d', num_classes),
            'resnext101_32x8d': lambda num_classes: TIMMModel('resnext101_32x8d', num_classes),
            ...
            'efficientnet_b2': lambda num_classes: TIMMModel('efficientnet_b2', num_classes),
            'efficientnet_b3': lambda num_classes: TIMMModel('efficientnet_b3', num_classes),
            'efficientnet_b4': lambda num_classes: TIMMModel('efficientnet_b4', num_classes),
            'efficientnet_b5': lambda num_classes: TIMMModel('efficientnet_b5', num_classes),
            'efficientnet_b6': lambda num_classes: TIMMModel('efficientnet_b6', num_classes),
            'efficientnet_b7': lambda num_classes: TIMMModel('efficientnet_b7', num_classes),

            # MobileNet family
            'mobilenetv2_100': lambda num_classes: TIMMModel('mobilenetv2_100', num_classes),
            'mobilenetv2_140': lambda num_classes: TIMMModel('mobilenetv2_140', num_classes),
            'mobilenetv3_small_100': lambda num_classes: TIMMModel('mobilenetv3_small_100', num_classes),
            'mobilenetv3_large_100': lambda num_classes: TIMMModel('mobilenetv3_large_100', num_classes),
            ...
            'vit_base_patch16_224': lambda num_classes: TIMMModel('vit_base_patch16_224', num_classes),

            # DenseNet family
            'densenet121': lambda num_classes: TIMMModel('densenet121', num_classes),
            'densenet169': lambda num_classes: TIMMModel('densenet169', num_classes),
            'densenet201': lambda num_classes: TIMMModel('densenet201', num_classes),
            'densenet161': lambda num_classes: TIMMModel('densenet161', num_classes),
        }
    ...
    def list_models_by_family(self):
        """List models organized by family"""
        families = {
            'ResNet': [m for m in self.available_models if m.startswith('resnet') or 'resnet' in m],
            'EfficientNet': [m for m in self.available_models if m.startswith('efficientnet')],
            'MobileNet': [m for m in self.available_models if m.startswith('mobilenet')],
            'Vision Transformer': [m for m in self.available_models if m.startswith('vit_')],
            'DenseNet': [m for m in self.available_models if m.startswith('densenet')],
        }
        return families
...
class ExperimentManager:
    ...
    def run_experiment(self, config):
        ...
        percentage_suffix = f"_{config['budget_percentage']}p" if 'budget_percentage' in config else f"_{config['budget']}"
        exp_name = f"{config['model']}_{config['dataset']}{percentage_suffix}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}{run_suffix}"
        ...
        # Calculate budget if percentage is provided
        if 'budget_percentage' in config:
            config['budget'] = int(len(train_dataset) * config['budget_percentage'] / 100)
    ...
    def run_multiple_experiments(self, configs, num_sequential_runs=3):
        ...
        for config_idx, config in enumerate(configs):
            ...
            print(f"Config: {config['model']} on {config['dataset']}, budget={config.get('budget_percentage', config.get('budget'))}")
        ...
        for config_idx, config in enumerate(configs):
            ...
            if config_results:
                ...
                summary_data.append({
                    ...
                    'budget': config.get('budget_percentage', config.get('budget')),
...
def validate(model, val_loader, loss_fn, device, writer=None, epoch=0, class_names=None):
    ...
    if writer:
        ...
        writer.add_scalar('Accuracy/best_val', avg_acc, epoch)  # Track best validation accuracy
...
def train_with_mascs(model, train_dataset, val_dataset, args, writer, exp_dir):
    ...
    mascs = MASCSOptimized(
        ...
        budget_percentage=args['budget_percentage'] if 'budget_percentage' in args else None,
...
def main():
    ...
    parser.add_argument('--budget', type=int, default=5000, help='Coreset budget (fixed number of samples)')
    parser.add_argument('--budget_percentage', type=float, help='Coreset budget as percentage of dataset')
    ...
    parser.add_argument('--percentages', nargs='+', type=float, help='List of percentages to run experiments on')
    parser.add_argument('--datasets', nargs='+', type=str, help='List of datasets to run experiments on')
    parser.add_argument('--models', nargs='+', type=str, help='List of models to run experiments on')
    parser.add_argument('--list_models', action='store_true', help='List available models by family')
    parser.add_argument('--list_datasets', action='store_true', help='List available datasets')
    parser.add_argument('--experiment_config', type=str, help='Path to JSON file with experiment configurations')
    ...
    if args.list_models:
        ...
        families = model_manager.list_models_by_family()
    ...
    if args.list_datasets:
        ...
        for dataset in sorted(dataset_manager.available_datasets.keys()):
            print(f"  - {dataset}")
        return

    if args.run_multiple:
        # Generate configurations based on provided arguments
        configs = []
        ...
        if args.experiment_config:
            ...
            # Generate configs from command line arguments
            datasets_to_run = args.datasets if args.datasets else [args.dataset]
            models_to_run = args.models if args.models else [args.model]
            percentages_to_run = args.percentages if args.percentages else [10, 20, 30, 50, 70, 100]
        ...
        print(f"Running {len(configs)} configurations with {args.num_sequential_runs} runs each")
        ...
        # Use percentage if provided, otherwise use fixed budget
        if args.budget_percentage:
            config['budget_percentage'] = args.budget_percentage
        else:
