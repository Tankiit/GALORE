...
def train_with_mascs(model, train_dataset, val_dataset, args):
    ...
    for epoch in range(args.epochs):
        ...
        coreset_loader = torch.utils.data.DataLoader(
            coreset_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True
        )
...
def main():
    parser = argparse.ArgumentParser(description='MASCS for CIFAR-10 with TIMM')
